...
Note that if you just invert $x^2$ and $xy$, then this does not give a cover $\P^1$, since the the radical of $(x^2,xy)$ does not contain the irrelevant ideal. Geometrically speaking, this is because inverting $xy$ corresponds to the affine open of $\P^1$ obtained by removing $0$ and $\infty$.

\subsection{Closed subschemes of \texorpdfstring{$\P^n$}{Pn}} Just as in closed subschemes of an affine scheme $\Spec R$, are given by ideals $I\subset R$, closed subschemes of $\P^n$ correspond to homogeneous ideals of $k[x_0,\ldots,x_n]$. To prove this fact, we will first note a few observation about line bundles $\sO(n)$'s on $\P^n$. Just as in the case of $\P^1$, we can define the line bundles $\sO(n)$'s whose global sections are degree $n$ polynomials. Therefore, we see that $k[x_0,\ldots,x_n]=\oplus_{i\geq 0}\Gamma (\sO(n),\P^n)$.
...
\[0\rightarrow I_{x_i}\rightarrow k[x_{0/i},x_{1/i},\ldots,\ldots,x_{n/i}]/(x_{i/i}-1). \]

Let $\lbrace f_{j/i}\rbrace_j$ be generators of $I_{x_i}$. Multiplying by each $f_{j/i}$ by a sufficiently large power of $x_i$, gives us homogeneous polynomials, $f_j$'s.
