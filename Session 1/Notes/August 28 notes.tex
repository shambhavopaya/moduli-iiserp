...
These are notes for the first few lectures. The aim is to gather sufficient algebraic geometry background for discussing moduli theory. A lot of \LaTeX\, code in this document has been shamelessly copied from the stacks project repository on GitHub.
\begin{comment}
\section{Plan}
...
\subsection{Two ways of Gluing $\A^1\setminus \lbrace 0\rbrace$}
...
\begin{example}
The second choice gives us the pojective line $\P^1$. This is looks like $\A^1$ with ``a point added at infinity". We will now compute its global sections.
...
All the above are examples of normal, reduced and Noetherian scheme. An example of a non-Noetherian scheme is $\Spec R[x_1,x_2,\ldots]$.

\subsection{A slightly more involved scheme: $\P^n$.}
Let $D(x_i):= k[x_{0/i},x_{1/i},\ldots,\hat{x_{i/i}},\ldots,x_{n/i}]$. And, we have a map $D(x_i)_{x_{j/i}}\rightarrow D(x_j)_{x_{i/j}}$ given by $x_{k/i}\mapsto x_{k/j}$\footnote{If you have seen the construction of Grassmannians as smooth manifolds, the same construction also goes through in algebraic geometry.}
...
\subsection{Line Bundles on $\P^1$:}
...
[Discussed till here as of \today]
...
\item Blow-up of $\A^2$ at the origin. (because everyone should know about blow-ups!)
\item An example of a scheme without a closed point.
\end{enumerate}
