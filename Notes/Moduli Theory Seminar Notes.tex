...
\begin{example}[A representable diagonal] 
	Let $F$ be the functor on $\mathit{Sch/k}$ defined in Example \ref{functor-of-conics}. The diagonal of this functor $\Delta: F\rightarrow F\times_k F$ is representable by $k$-schemes! I will add a proof soon. These is essentially because we have flat families. Moduli problems dealing with non-flat families are almost impossible to work with.
