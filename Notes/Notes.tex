...
Let $k$ be a field and $\sF$ a coherent sheaf on $\P_k^n$. By similar arguments as in \ref{subsection-closed-in-proj}, we can show that $\sF$ corresponds to a graded module $M_{\bullet}$ over the graded ring $S:= k[X_0,X_1,\ldots, X_n]$. We will the hilbert function for $\sF$ in terms of the associated graded module.
...
We call the associated polynomial as the Hilbert polynomial of $M$. For a closed subscheme $X\subseteq \P_k^n$, the Hilbert polynomial of $X$ is defined to be the Hilbert polynomial of the graded module $\Gamme (X):=S/I$, where $I$ is the graded ideal of $X$.
We will now compute the Hilbert function in some concrete examples.

\begin{example}
	\begin{enumerate}
		\item ($d$ points)
		\item $d$-fold Veronese
