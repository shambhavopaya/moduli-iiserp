...
\begin{example}
	...
	Choose another smooth elliptic curve $E'$ such that $j(E')\neq j(E)$. Both $E$ and $E'$ are degree $3$ curves in $\P^2_\C$, so have the same Hilbert polynomial $h_E=h_{E'}=3m$, but are not isomorphic as their $j$-invariants are different\footnote{This example was pointed out to me by Suraj Yadav.}.
\end{example}
...
\begin{example}[Grassmannian of a coherent sheaf]
	\label{example-grassmannian-coherent-sheaf}\marginnote{001W}
...
\begin{proof}
	...
	\begin{center}
		\begin{tikzcd}
		G(E,d)_U\arrow[r]\arrow[d] & G(E,d)\arrow[d]\\
		U\arrow[r] & S
		\end{tikzcd}
	\end{center}
...
\begin{lemma}[Vanishing of a homomorphism is a closed condition]
	\label{lemma-vanishing-locus-module-homomorphism}\marginnote{001X}
...
[Please refer to \cite[\S 5.2]{FGAExplained}]
...
\begin{theorem}[Mumford]
	...
	For any coherent subsheaf $\sF\subset \oplus^p\, \sO_{\P^n_k}$ with Hilbert polynomial 
	$\chi(\sF(r))=\overset{n}{\underset{i=0}{\sum}} a_i 
	\begin{psmallmatrix}
	r\\
	i	
	\end{psmallmatrix}$,
	$\sF$ is $F_{p,n}(a_0,\ldots,a_n)$-regular.
\end{theorem}
